...
@Component({
  selector: 'app-balance-sheet-accounts',
  templateUrl: './balance-sheet-accounts.component.html',
  styleUrls: ['./balance-sheet-accounts.component.css']
})
export class BalanceSheetAccountsComponent implements OnInit {
  ...
  @Input() public balanceCount!: (param: any) => void;
  modalRef: MdbModalRef<CaptureModalComponent> | null = null;
  public balanceSheetAccountCount: any;
  ...
  public count: string = '';
  public balancesheetcount: string = '';
  ...
  ngOnChanges(): void {
    this.fetchAccountData();
    this.balanceSheetCount();
    this.balanceSheetBalance();
  }
  ...
  fetchAccountData() {
    ...
    const year = this.selectedYear ? this.selectedYear : currentYear;
    const month = this.selectedMonth ? this.selectedMonth : currentMonth;
    const param = 'year=' + year + '&month=' + month + '&company_id=' + company_id;
    ...
    this.accountService.getBalanceSheet(year, month, company_id).subscribe((res: any) => {
  ...
  openModal(event, accountId) {
    ...
    const year = this.selectedYear ? this.selectedYear : this.currentYear;
  ...
  openBulkCertifyModal(event) {
    ...
    const title = 'Bulk Certify Balance Statement';
    const year = this.selectedYear ? this.selectedYear : this.currentYear;
    const month = this.selectedMonth ? this.selectedMonth : this.currentMonth;
  ...
  openBulkReviewModal(event) {
    const companyId = this.selectedCompanyID;
    const account_details = this.accountData
    const title = 'Bulk Review Balance Statement';
    const year = this.selectedYear ? this.selectedYear : this.currentYear;
    const month = this.selectedMonth ? this.selectedMonth : this.currentMonth;
  ...
  balanceSheetCount() {
    ...
    const year = this.selectedYear ? this.selectedYear : this.currentYear;
    const month = this.selectedMonth ? this.selectedMonth : this.currentMonth;
    this.accountService.balanceSheetCount(year, month, companyId).subscribe({
      next: (res: any) => {
  ...
  balanceSheetBalance() {
    ...
    const year = this.selectedYear ? this.selectedYear : this.currentYear;
    const month = this.selectedMonth ? this.selectedMonth : this.currentMonth;
    this.accountService.balanceSheetBalance(year, month, companyId).subscribe({
      next: (res: any) => {
        this.balancesheetcount = res.message;
      },
      error: (error => {
        this.handleError(error);
      })
